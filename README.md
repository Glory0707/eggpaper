...
- **页边即骨架**：一列**角色书签**（背/缺/主/证/对/流/拓/限，首字+色块），点开即该段的角色卡——写全称、写出处（¶号+页码）、可改判、可回退；略读蒙纱也按同一套角色判断
...
- **导出**：阅读笔记一键导出 Markdown（一眼卡 + 论证骨架 + 段落角色汇总 + 眉批查译）、术语表导出 CSV（Anki 也能直接导这个）
...
- **右栏四分工**：问题（六个问题 + 眉批速览）· 速览（一眼卡 + 图表 + 方法卡 + 导师三问）· 提问 · 术语（本文缩写 + 术语表）；**宽度可拖**（双击复位，268–760px），面板可以整个收起。分工按"谁负责哪一类信息"划死：一眼卡只做三十秒定位，「为什么重要」在问题②、「依据在哪」在③、「作者承认了什么」在④，速览里只留一个 ↗ 指针

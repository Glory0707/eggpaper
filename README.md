...
- **略读模式**（f）：背景/样板/参考文献**整段文字本体变灰**（灰里没有蒙版也没有黑点，符号一起灰），
  图注、首页、有批注的段不灰；留下的核心段在页边一道芯线，`j/k` 只在主干上跳；悬停灰字回黑看一眼，点一下=这段也要读。
  **综述只灰参考文献**——综述的文献梳理本身就是正文，不蒙（导入时自动判类型，研究型/综述型）
...
- **问题**（右栏第一个页签）：读一篇论文该带着的问题——`要解决什么 / 为什么要解决 / 怎么解决的 / 还有什么没解决 / 还能做什么 / 换个学科怎么看`。**六问随首次析读一次备齐**（要点进对应段 + 模型作答，按篇缓存），换篇即换一套；还能做什么 / 换个学科两问的每条追问，一点就带进提问面板问出去。**综述自动换口径**：③变成「它把文献怎么组织的？」——按什么线索分类、各块什么关系、落到哪些开放问题
- **页边只有两样东西**：批注卡（眉批与查译，钉在它引的那句话旁）和引文划线。段落角色不再在页边上挂牌子——它退到后台只干一件事：**略读**时决定哪几段变灰、哪几段留一条核心条
...
- **右栏四分工**：问题（六个问题；眉批只留一个生成入口，内容在纸面）· 速览（一眼卡 + 图表 + 方法卡 + 导师三问）· 提问 · 术语（本文缩写 + 术语表）；**宽度可拖**（双击复位，268–760px），面板可以整个收起。**同屏 2–4 篇对照读**时右栏改浮层——展开盖上来、收起还整屏给文献，不再是死按钮。分工按"谁负责哪一类信息"划死：一眼卡只做三十秒定位，「为什么重要」在问题②、「依据在哪」在③、「作者承认了什么」在④，速览里只留一个 ↗ 指针
...
- **整本双语导出**：pdf2zh 引擎（bing/google/deepl/openai 可配），奇页原文偶页译文；**按页流水线**——
  两页并行、单页超时自动重试，坏页回退原文并如实标注，一页卡不住整本；译完后按钮变「重新整本翻译」，
  译文打不开点一下就重译

### 键盘流

`j/k` 段间步进 · `PageUp/PageDown` 翻页 · `Ctrl+F` 论文内查找 · `t` 译当前段并钉页边 · `s` 译划选 · `f` 略读 · `r` 框选问 AI · `1/2/3` 三模式 · `/` 提问 · `a` 析读 · `m` AI 眉批 · `x` 折叠右栏 · `g l` 文库 · `Alt+←` 返回原位 · `Esc` 收起所有浮层/退出框选 · `?` 快捷键卡

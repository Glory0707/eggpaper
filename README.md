...
- **论证骨架**：结构条一眼看全段落构成；缺口 → 主张 → 证据链排版化列表，跟随滚动高亮"你读到哪条主张"；每个标签可溯源依据句、可改判、可回退
...
- **框选视觉问答**：`r` 进入框选模式，圈住公式/图表/表格直接问（讲解此图 / 讲公式 / 挖表格），可钉在页边；图表灯箱里也能「问这张图」
...
- **导出**：阅读笔记一键导出 Markdown（一眼卡+论证骨架+眉批查译）、术语表导出 CSV / Anki TSV / Anki TSV

### 沉淀

- **术语本**：内置化学/材料种子术语；划词一键收进；译名在所有输出中强制一致；筛选管理
- **页边即资产**：查译、段译、眉批都钉在页边（统一卡片、可展开、可移除、同段重钉自动更新），跟随论文保存

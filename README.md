...
1. **术语一致的翻译**——一篇一份术语表，贯穿划词、段落、整本、总结、问答全部链路
...
- **整本双语导出**：pdf2zh 引擎（bing/google/deepl/openai 可配），奇页原文偶页译文；**按批流水线**——
  四页一批、批内坏页拆单兜底、超时自动重试，坏页回退原文并如实标注，一页卡不住整本；
  中断续译不用重花钱。译完后按钮变「重新整本翻译」，译文打不开点一下就重译

...
- **对话式提问**（右栏「提问」，照 DeepSeek / 豆包网页版的样子做）：回答**逐字流式**吐出来，中途可「停止生成」（已经吐出来的半截留着）；每条消息悬停出「复制 / 重新生成 / 删除」；**一摊对话一个上下文**，可新建 / 切换 / 改名 / 删掉整摊，标题自动取第一个问题。答案里的 ¶ 号与末尾「依据」角标都可点回原文
...
- **文库**：分类（Zotero 的 collection 语义——一篇可同时属于多类，点分类即筛选，条目可直接拖进去或从右上角数字勾选）+ 标题搜索 + 三种排序（最近导入 / **最近阅读** / 标题）。批注、骨架、问答全部落在本机 SQLite，下次打开原样都在

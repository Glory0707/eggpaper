...
def _messages(title: str, corpus: str, claims: list, annos: dict) -> list:
    role_note = ""
    ...
    return [
        ...
        {"role": "user", "content":
            f"论文《{title}》。为数据对比表抽取以下六个维度，每个维度给一段"
            f"（≤60字，关键结果优先带数字），并给出依据所在的段落号——ref 必须是**整数**"
            f"（如 3 代表 ¶3；确实定位不到才写 null）。\n"
            f"维度说明：{'；'.join(f'{k}={hint}' for k, _n, hint in DIMS)}。\n"
            f"输出 JSON：{schema}\n"
            f"{role_note}\n\n【正文】\n{corpus}"},
    ]
...
def _mock(paper: dict) -> dict:
    """演示模式：一眼假但结构完整的格子，别让演示用户等一场真实的模型调用。"""
    t = (paper.get("title") or paper.get("filename") or "这篇文献")[:18]
    cells = {}
    for i, k in enumerate(KEYS):
        cells[k] = {"text": f"（演示）《{t}》的{'研究问题 方法 实验体系 数据 关键结果 局限'.split()[i]}示意内容",
                    "ref": 2 + i * 3}
    return cells


def extract_all(papers: list, material_of, demo: bool = False) -> dict:
    """papers: [{id,title,...}]；material_of(pid) -> (paras, claims, annos)。
    返回 {pid: {dim: {text, ref}}}。每篇一线程并行，谁失败谁一格降级，不拖垮整张表。"""
    out = {}
    if demo:
        return {p["id"]: _mock(p) for p in papers}

    def one(p):
        paras, claims, annos = material_of(p["id"])
        msgs = _messages(p.get("title") or p.get("filename") or "", _corpus(paras), claims, annos)
        try:
            return p["id"], _norm(chat_json(msgs, max_tokens=1600))
        except Exception:

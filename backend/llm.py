...
def suggest_questions(title: str, claims: list, annos: dict) -> dict:
    ...
    out = chat([
        {"role": "system", "content":
            ...
            "要具体到这篇的内容（怎么做的、数字在什么条件下得的、这个结论能不能用到别的体系、"
            "某个术语在这里到底指什么），是他读完想弄明白而不是想挑刺的那种问题。"
...
def summarize(title: str, paras: list, hits=None) -> dict:
    ...
    out = chat([
        {"role": "system", "content": _gloss_block(hits) +
            ...
            '"findings":"<发现：只留最硬的一到两个数据结论，带关键数字，≤60字——'
            '二级结论（次要的、局部的那些）不要写进来，它们归主张-证据链>",'
            '"keywords":["<3~5个中文关键词>"]}'
...
def advisor_questions(title: str, claims: list, warnings: list) -> dict:
    ...
    out = chat([
        {"role": "system", "content":
            # 信息分工：④ 收"作者自己承认 / 眉批已经标出的薄弱点"，这里只收"再狠一层"。
            # 两边素材同源（都是主张 + 眉批有坑），所以必须在提示词里把边界写死：
            # 已经认了的不是问题，是前提——答辩现场没人会因为你复述"我承认采样有限"而加分。
            "你是苛刻但建设性的导师。学生要拿这篇论文去组会汇报/答辩。"
            "下面给你的『已承认的薄弱点』是作者自己在文里承认、或眉批已经标出来的——"
            "**不要**再问这些（复述它们等于没做准备）。"
            "请在这之上往前推一层：为什么'承认了'还不够？缺的是哪一步证据？"
            "结论到底能走到哪一步？从证据强度、方法选择、结论推广性三个方向里出 3 个"
            "最可能把学生问住的问题，每个问题配一份过关要点提纲。"
...
def answer_next(title: str, limits: list, exts: list, claims: list, warns: list) -> dict:
    ...
    out = chat([
        {"role": "system", "content":
            ...
            "该换哪种方法），每条一句话说完别铺陈；禁止'进一步研究''拓宽应用'这类空话。"
            "注意：这一栏是**建设性的下一步**（顺着它往下走能拿到什么），不写成审稿人挑刺，"
            "也不复述已标出的可疑之处本身——那是别处的活。"
            "每条配一句能直接拿去问模型的追问。"
            '只输出 JSON：{"items":[{"lead":"<方向名，≤8字>",'
            '"text":"<做什么、为什么，≤52字，句尾带依据段号 [¶n]，没有依据就不标>",'
...
def answer_lens(title: str, one_line: str, claims: list, paras: list) -> dict:
    ...
    out = chat([
        {"role": "system", "content":
            ...
            "另外两件不要做：不要复述这篇论文已知的局限或可疑之处（别处已经说过），"
            "也不要下'结论存疑'这类判决——只写**这一行的人会盯什么、会要什么证据**。"
            '只输出 JSON：{"items":[{"lead":"<领域名，≤10字>",'
            '"text":"<他会盯这篇的哪一点、为什么，≤52字>",'

...
RING_RATIO = 0.085  # 环宽 = 蛋高的这个比例
BARS_BY_SIZE = ((56, 4), (40, 4), (26, 3), (0, 2))   # 尺寸 ≥ 此值 → 用几根字条
```

几何是**按目标像素算出来的**，不是手调的尺寸表：缝 `g` 由 `环内高度 / (2n+1)` 解出，
放不下就减字条（4→3→2），绝不压窄缝；字条宽度按椭圆在**它最远那条边**处的内接宽度收。

> **判据只有一条：白缝。** 小尺寸上"糊不糊"取决于笔画之间还剩多少空隙，不是"笔画够不够粗"。
> 曾经改过一版"更壮"的（椭圆撑满、环加粗），把缝压到 1px 以下，整个图标糊成一坨深色——
> 用户的原话是"改了又糊了"。所以：**手调数字这件事已经被禁止了，改几何只能改上面这几个比例。**

改完必须过闸门（会非零退出）：

```bash
python tools/check_icon.py     # 实测 16~64px 各档的白缝，低于 max(1.6px, min(画布7.5%, 3px)) 就报错
```

它会打印每一档的缝宽/环宽/字条数，并输出最近邻放大对照表 `_qa/shots/icon_px.png`，
还附 ASCII 放大图——**先看这张图和数字，再谈好不好看。**

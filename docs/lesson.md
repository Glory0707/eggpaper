...
- **AI 上下文缓存吃的是「从第 0 个 token 起逐字节相同」的前缀**：同篇论文的整文任务共用 SHARED_SYSTEM + paper_doc（统一每段 1200 字、全文 90k），任务规则放全文之后——命中输入按约 1/50 计费。任何随任务/调用变化的字段（图表注、术语表、问题、历史）进了前缀就前功尽弃。

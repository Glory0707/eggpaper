...
| `--fs-read` | 14.5px | **只给"真的要读进去"的整段话**：六问答案、证据说明、一眼卡正文、聊天正文 |
...
10. **问题先出现，答案点开才看**：右栏骨架页签是六行问题（要解决什么 / 为什么要解决 / 怎么解决的 / 还有什么没解决 / 还能做什么 / 换个学科怎么看），默认全部收起——读一篇论文该带着问题进去，一打开就把结论摆出来是替人读完了。收起时右侧给一个读数（`5` = 里面有条目），不点开也知道值不值得点。三问的答案白送（缺口段 / 主张-证据链 / 局限段 + 眉批"有坑"），三问点「获取」才写，每条追问一点就带进提问面板。
...
### 6.1 一条信息只住一个房间（M3.20 定的分工）

文案之外还有一层重复：同一个判断被搬到两三个页签里各说一遍。它比文案啰嗦更伤——读者会以为那是两条不同的信息，于是重新读一遍。规则：

- **一个判断只在一个地方展开，别处只给指针。** 一眼卡只写结论（一句话 + 发现 + 待解），想深了顺着 `↗` 走：发现→问题③、待解→问题④。**指针是"去那儿看"，不是"这儿再说一遍"**。
- **跨模块去重不可能靠"提示词里写一句别重复"。** 两条生成链路互不知情，素材同源（都是主张 + 眉批有坑）就必然撞车。唯一的解法是**信息分工**：④＝作者承认的与眉批已标的、⑤＝顺着往下能做的、导师三问＝在这之上再狠一层。三处的输入与口径都在提示词里写死。
- **同一批文字的两种视图，只能留一个。** 眉批「有坑」只在问题④出现（眉批速览里排除）；「依据 ¶n」只由正文里那些可点的 ¶ 承担（回答底部不再复述一遍）；缩写收进术语表之后就从"本文缩写"里消失。
- **算过的东西进页面就显示**（读缓存不花钱），但**没算过的绝不偷偷算**——「获取」是一次用户授权的模型调用，不是一次页面渲染的副作用。

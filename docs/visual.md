...
10. **读段提示：读到哪一段，就说一句它是什么**。视口中心那一段一换，纸下缘轻轻报一句 `¶8 · 标准流程` 并给两个真动作（译这段 / 问这段）。这是"让科研人真的能读进去"的那一下——一个人读 PDF 时最缺的，就是"我现在读的这句重要吗、能不能马上问一句"。**同一段里来回滚不再弹**（否则它就成了跟着你的小广告），4.2 秒自己走；想看它回来，往下读一段就有，或者直接按 `t` / `/`。
11. **阅读进度是一根发丝**：贴书桌右缘的一根 3px 细线，读到哪长到哪，不写字、不报数（页码在控件条上已经有了）。
...
| 读完一段 | 纸下缘的读段提示浮出来（popIn 200ms），4.2s 后自己淡出 |
